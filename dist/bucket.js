...
module.require = function require(url, optionsOrDone, done) {
    ...
    if (!item) {
        this.fetch(url, { sync: options.sync, timeout: options.timeout || 5000 }, function(err, result) {
            ...
            module.append(module.createElement(
                err ? url : result,
                { defer: options.defer, style: isCss, link: !!err }), options.bottom);

            done && done();
        });
    } else {
        this.append(this.createElement(item, { defer: options.defer, style: isCss, link: !!options.link }), options.bottom);
        done && done();
    }
};
